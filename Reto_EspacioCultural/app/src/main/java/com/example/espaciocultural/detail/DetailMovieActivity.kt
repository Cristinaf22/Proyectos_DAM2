...
class DetailMovieActivity : AppCompatActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        var event = listEvents[positionEvent] as MovieEvent
        ...
        val resultLauncher = registerForActivityResult(ActivityResultContracts.StartActivityForResult()) { _ ->
            ...
            event = listEvents[positionEvent] as MovieEvent
            addEventData(event)
        }
    ...
    private fun addEventData(event : MovieEvent)
    {
        ...
        val directorName = findViewById<TextView>(R.id.eventDirectorName)
        directorName.text = event.directorName

        val premierYear = findViewById<TextView>(R.id.eventPremierYear)
        premierYear.text = event.premiereYear.toString()

        val mainActors = findViewById<TextView>(R.id.eventMainActorList)
        mainActors.text = event.mainActors.joinToString(",")

        val durationMovieEvent = findViewById<TextView>(R.id.eventDuration)
        durationMovieEvent.text = event.movieLength.toString()
    }
}